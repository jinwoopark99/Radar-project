...
def set_arm(arm):
    ...
    try:
        arming_service = rospy.ServiceProxy('/uav3/mavros/cmd/arming', CommandBool)
...
def set_offboard_mode():
    ...
    try:
        set_mode_service = rospy.ServiceProxy('/uav3/mavros/set_mode', SetMode)
...
if __name__ == '__main__':
    ...
    local_pos_pub = rospy.Publisher('/uav3/mavros/setpoint_position/local', PoseStamped, queue_size=100)

    # Subscriber to state topic
    rospy.Subscriber('/uav3/mavros/state', State, state_callback)
    rospy.Subscriber('/mission_start_3', Bool, mission_start_callback)
